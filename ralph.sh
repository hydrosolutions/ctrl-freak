...
#   ./ralph.sh tasks/prd-feature.json   # uses custom prd, 10 iterations
#   ./ralph.sh tasks/prd-feature.json 20  # uses custom prd, 20 iterations

set -e

# Parse arguments - first arg can be a .json file or a number
if [[ "$1" =~ \.json$ ]]; then
  PRD_FILE="$1"
  MAX_ITERATIONS=${2:-10}
elif [[ "$1" =~ ^[0-9]+$ ]]; then
...
# Derive progress file from PRD file (prd-feature.json -> progress-feature.txt)
